...
1. The command can communicate with the `Model` when it is executed (e.g. to delete a athlete).<br>
...
| `* * *`  | user                                                | find a athlete by name                | locate details of athletes without having to go through the entire list |
...
| `*`      | user with many athletes in the address book          | sort athletes by name                 | locate a athlete easily                                                 |
...
1. Deleting a athlete while all athletes are being shown
