 ...
 *
 * @see Athlete#isSamePerson(Athlete)
 */
public class UniquePersonList implements Iterable<Athlete> {
    ...
    public void setPerson(Athlete target, Athlete editedAthlete) {
        requireAllNonNull(target, editedAthlete);
        ...
        if (!target.isSamePerson(editedAthlete) && contains(editedAthlete)) {
            throw new DuplicatePersonException();
        }

        internalList.set(index, editedAthlete);
    }
     ...
     */
    public void setPersons(List<Athlete> athletes) {
        requireAllNonNull(athletes);
        if (!personsAreUnique(athletes)) {
            throw new DuplicatePersonException();
        }
     ...
     */
    private boolean personsAreUnique(List<Athlete> athletes) {
        for (int i = 0; i < athletes.size() - 1; i++) {
            for (int j = i + 1; j < athletes.size(); j++) {
                if (athletes.get(i).isSamePerson(athletes.get(j))) {
                    return false;
                }
            }
        }
